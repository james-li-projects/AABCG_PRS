...
###################################################
# computing tuning set AUCs
tuning_auc_list <- vector(mode="list",length=length(PRS_score_list))
num_snp_list <- c()
for (i in 1:length(PRS_score_list)) {
  print(i)
  tmp_score_cov <- inner_join(imported_scores[[i]],y_tune,by=c("#IID"))
  ...
  output_AROC.sp <- AROC.sp(formula.h = PRS~Age+factor(Platform)+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10,
                            ...
                            data = tmp_score_cov,
                            tag.h = 0)
  tuning_auc_list[[i]] <- sort(as.numeric(output_AROC.sp$AUC))
...
# extracting tuning set AUCs into a vector and ultimately into a data.frame
tuning_auc_vec_lower <- c()
tuning_auc_vec_center <- c()
tuning_auc_vec_upper <- c()
for (i in 1:length(tuning_auc_list)) {
  print(i)
  tuning_auc_vec_lower <- c(tuning_auc_vec_lower, tuning_auc_list[[i]][1])
  tuning_auc_vec_center <- c(tuning_auc_vec_center, tuning_auc_list[[i]][2])
  tuning_auc_vec_upper <- c(tuning_auc_vec_upper, tuning_auc_list[[i]][3])
}
tuning_AUC_df <- data.frame(colnames(combined_PRS_scores)[-1],tuning_auc_vec_lower,tuning_auc_vec_center,tuning_auc_vec_upper,num_snp_list)
colnames(tuning_AUC_df) <- c("Approach","AUC_lower","AUC_center","AUC_upper","Num_SNP")
print(tuning_AUC_df %>% arrange(desc(AUC_center)))
...
validation_AUC_df <- data.frame(colnames(combined_PRS_scores)[-1],validation_auc_vec_lower,validation_auc_vec_center,validation_auc_vec_upper,num_snp_list)
colnames(validation_AUC_df) <- c("Approach","AUC_lower","AUC_center","AUC_upper","Num_SNP")
print(validation_AUC_df %>% arrange(desc(AUC_center)))
save(validation_AUC_df,file=paste0("/gpfs/data/huo-lab/BCAC/james.li/PRS_AABCG/output/SCORE_AUC/",subtype,"_validation_AUC_df.RData"))
...
SUBTYPE_SPECIFIC_AFR_SCORES <- combined_PRS_scores %>%
  ...
  select_if(!grepl("EUR", names(.))) %>% mutate(`#IID` = rownames(.))
...
for (i in c(first_col_index_XANCESTRY_PRS:last_col_index_XANCESTRY_PRS)) {
  ...
  tmp_approach_snp_list_EUR <- fread(paste0("../../SCORE_FILES_PROCESSED/",tmp_EUR_PRS_NAME),header=F)
...
save(XANCESTRY_AUC_df,file=paste0("/gpfs/data/huo-lab/BCAC/james.li/PRS_AABCG/output/SCORE_AUC/",subtype,"_XANCESTRY_AUC_df.RData"))
...
for (row_index in 1:nrow(XANCESTRY_COEF_VEC)) {
  ...
  write.table(COLLATED_SCORE_FILE_DF,file=paste0("../../FINAL_PRS_MODELS/",subtype,"_XANCESTRY_",XANCESTRY_APPROACH_STRING),quote=F,row.names=F,col.names=F,sep="\t")
}
