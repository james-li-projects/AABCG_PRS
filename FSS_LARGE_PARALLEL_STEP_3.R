...
# extracting lists of selected SNPs from the first window orientation 
all_p_thresh_files <- list.files()[grep(p_thresh_string,list.files())]
SELECTION_1_SNP_FILES <- all_p_thresh_files[grep("SELECTION_1_SNP_LIST_CHROM_WINDOW_SNP_LIST_pthresh_",all_p_thresh_files)]
ENTIRE_SELECTION_1_SNP_LIST <- data.table(ID = as.character())
for (snp_file in SELECTION_1_SNP_FILES) {
  load(snp_file)
  ENTIRE_SELECTION_1_SNP_LIST <- rbind(ENTIRE_SELECTION_1_SNP_LIST, SELECTION_1_SNP_LIST)
}

# filtering sumstats to only contain these selected SNPs from the first window orientation 
sumstats <- sumstats %>% filter(ID %in% ENTIRE_SELECTION_1_SNP_LIST$ID)
...
for (CHROM_INDEX in CHROM_INDEX_LIST) {
  ...
  # creating a modified position value for each SNP that accounts for chromosomes being far apart prior to performing LD clumping while offsetting these windows by 2.5 Mb
  ...
  significant_SNP_df_modified <- significant_SNP_df_modified %>% mutate(modified_POS = `#CHROM`*(1e14) + POS + 2500000)
  ...
  for (index in seq(TOTAL_WINDOW_NUM)) {
    ...
    write.table(CHROM_WINDOW_SNP_LIST,file = paste0("OFFSET_CHROM_WINDOW_SNP_LIST_pthresh_",p_thresh_string,"_chr_",CHROM_INDEX,"_window_",index,".txt"),quote=F,row.names=F,col.names=F)
    print(paste0("OFFSET_CHROM_WINDOW_SNP_LIST_pthresh_",p_thresh_string,"_chr_",CHROM_INDEX,"_window_",index))
  }
}
